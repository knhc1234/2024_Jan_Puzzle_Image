    ...
    def __getitem__(self, idx):
        if self.mode == 'train':
            ...
            if self.transform != None:
                ...
                image_reshuffle = self.transform(image_reshuffle)
...
for i in range(1):
    augmented_train_dataset = JigsawDataset(
        df=train_df,
        data_path='./data',
        mode='train',
    ...
    if i == 0:
        combined_train_dataset = torch.utils.data.ConcatDataset([augmented_train_dataset, train_dataset])
    else:
        combined_train_dataset = torch.utils.data.ConcatDataset([combined_train_dataset, augmented_train_dataset])

combined_train_dataloader = DataLoader(combined_train_dataset, collate_fn=JigsawCollateFn(transform, 'train'), batch_size=config['batch_size'])
...
class LitJigsawElectra(L.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        '''
        scheduler = InverseSqrtScheduler(opt, self.hparams.warmup_steps)
        sch_config = {
            "scheduler": scheduler,
            "interval": "epoch",
    ...
    def training_step(self, batch):
        ...
        #Tensorboard 기록
        #writer.add_scalar('Train/loss_local', loss_local.item(), self.global_step)
        #writer.add_scalar('Train/loss_global', loss_global.item(), self.global_step)
    ...
    def validation_step(self, batch):
        ...
        #writer.add_scalar('Validation/loss_local', loss_local.item(), self.global_step)
        #writer.add_scalar('Validation/acc_local', local_accuracy.item(), self.global_step)
        #writer.add_scalar('Validation/loss_global', loss_global.item(), self.global_step)
...
trainer = L.Trainer(accelerator="gpu", max_epochs=1, precision='16-mixed', callbacks=[checkpoint_callback, earlystopping_callback])
#trainer = L.Trainer(accelerator="gpu", max_epochs=200, precision='bf16-mixed', callbacks=[checkpoint_callback, earlystopping_callback])

#trainer.fit(lit_jigsaw_electra, train_dataloader, val_dataloader) 
trainer.fit(lit_jigsaw_electra, combined_train_dataloader, val_dataloader) 

# TensorBoard writer를 flush하고 닫는 부분 추가
#writer.flush()
#writer.close()
