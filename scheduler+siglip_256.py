...
model = timm.create_model('vit_large_patch16_siglip_256', pretrained=True, num_classes=0)
...
train_dataloader = DataLoader(train_dataset, collate_fn=JigsawCollateFn(transform, 'train'), batch_size=config['batch_size'], num_workers=0)
val_dataloader = DataLoader(val_dataset, collate_fn=JigsawCollateFn(transform, 'val'), batch_size=config['batch_size'], num_workers=0)
...
checkpoint_callback = ModelCheckpoint(
    ...
    mode='max',
    dirpath='./checkpoints/',
    filename='jigsawelectra-vit_g_gap-{epoch:02d}-{val_score:.4f}',
    save_top_k=3,
